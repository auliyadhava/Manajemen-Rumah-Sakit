                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <?php if(empty($pending)): ?>
                                <tr><td colspan="5" class="text-center py-5 text-muted">Tidak ada antrian pembayaran.</td></tr>
                            <?php else: ?>
                                <?php foreach($pending as $p): ?>
                                <tr>
                                    <td class="fw-bold"><?= $p['full_name']; ?></td> 
                                    <td><?= $p['poli']; ?></td>
                                    <td class="text-primary fw-bold">Rp <?= number_format($p['total_amount'], 0, ',', '.'); ?></td>
                                    <td><span class="badge bg-warning text-dark px-3 py-2 rounded-pill"><?= strtoupper($p['payment_status']); ?></span></td>
                                    <td class="text-center">
                                        <button class="btn btn-success btn-sm px-3" data-bs-toggle="modal" data-bs-target="#modalBayar<?= $p['payment_id']; ?>">
                                            Proses Bayar 
                                        </button>
                                    </td>
                                </tr>

                                <div class="modal fade" id="modalBayar<?= $p['payment_id']; ?>" tabindex="-1">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <form action="/kasir/prosesBayar/<?= $p['payment_id']; ?>" method="post" style="width: 100%;">
                                            <div class="modal-content border-0">
                                                ...
                                                <div class="modal-body p-4">
                                                    <p class="text-muted mb-1">Total Tagihan Pasien:</p>
                                                    <h3 class="text-success fw-bold mb-4">Rp <?= number_format($p['total_amount'], 0, ',', '.'); ?></h3>
                                                ...
                                                <div class="modal-footer border-0">
                                                    ...
                                                    <button type="submit" class="btn btn-success px-4">Selesaikan Pembayaran</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
